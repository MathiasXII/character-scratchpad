...
fn context_file_workflow() {
    ...
    let created = create_context_file(character_dir.to_string_lossy().to_string(), "notes".to_string(), work_folder.clone())
        .expect("context file should be created");
    assert!(PathBuf::from(&created).exists());

    let duplicate = create_context_file(character_dir.to_string_lossy().to_string(), "notes".to_string(), work_folder.clone())
        .expect_err("duplicate context file should fail");
    assert!(duplicate.to_string().contains("already exists"));

    let auto_text = create_context_file(character_dir.to_string_lossy().to_string(), "outline".to_string(), work_folder.clone())
        .expect("file without extension should default to .txt");
    assert!(auto_text.ends_with("outline.txt"));

    let context_files = list_context_files(character_dir.to_string_lossy().to_string(), work_folder.clone())
    ...
    delete_context_file(character_dir.to_string_lossy().to_string(), "notes.txt".to_string(), work_folder.clone())
    ...
    let copied_name = copy_file_to_context(external_file.to_string_lossy().to_string(), character_dir.to_string_lossy().to_string(), work_folder.clone())
    ...
    let copied_name_again = copy_file_to_context(external_file.to_string_lossy().to_string(), character_dir.to_string_lossy().to_string(), work_folder.clone())
    ...
    let unsupported_err = copy_file_to_context(unsupported_file.to_string_lossy().to_string(), character_dir.to_string_lossy().to_string(), work_folder.clone())
        .expect_err("unsupported file type should fail");
    assert!(unsupported_err.to_string().contains("not supported"));
}
